...
Item {
    ...
    // Re-bind the Tab shortcut to specifically re-acquire main window focus before triggering
    Shortcut {
        sequence: "Tab"
        onActivated: {
    ...
    // Force internal focus routing dynamically whenever the popup itself becomes visible inside StackView
    onVisibleChanged: {
        if (visible) {
            window.focus = false;
            window.focus = true;

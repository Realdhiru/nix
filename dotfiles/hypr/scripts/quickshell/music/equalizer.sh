...
# Load the written preset into the SINGLE service-managed EasyEffects.
# EE 8.x: launching `easyeffects -l <preset>` while an instance is already
# running spawns a secondary that hands the load off to the primary over
# its local socket and exits — no duplicate instances. `pgrep -x
# easyeffects` cannot be used for liveness here: the Nix wrapper renames
# the comm to `.easyeffects-wr`, so `systemctl --user is-active` is the
# only reliable probe.
load_preset() {
    if ! systemctl --user is-active --quiet easyeffects; then
        systemctl --user start easyeffects >/dev/null 2>&1
        for _ in $(seq 1 20); do
            systemctl --user is-active --quiet easyeffects && break
            sleep 0.25
        done
    fi
    # The systemd service might be active but the local socket isn't ready yet.
    # We must loop and retry the load command until it succeeds.
    for _ in $(seq 1 20); do
